    ...
    for host in root.iter("host"):
        ...
        for port in host.iter("port"):
            ...
            out.append(Finding(
                ...
                title=f"Open port {portid}/{svc_name or 'tcp'}",
                severity="info",
                endpoint=f"{addr}:{portid}",
                description=f"Open port {portid} running {svc_name} on {addr} "
                            f"(exposure surface for the attack-path model).",
                evidence=f"{addr}:{portid} {svc_name}",
                raw={"port": portid, "service": svc_name, "host": addr},
            ))
    return out
...
def parse_openvas_xml(text: str, product: str) -> List[Finding]:
    ...
    for result in root.iter("result"):
        ...
        out.append(Finding(
            ...
            title=name,
            severity=normalize_severity(float(severity) >= 9.0 and "critical" or
                                        (float(severity) >= 7.0 and "high" or
                                         (float(severity) >= 4.0 and "medium" or "low"))),
            cve=cve, cwe=cwe,
            endpoint=host,
            description=result.findtext("description") or "",
            remediation=result.findtext("solution") or "",
            evidence=f"{host} severity={severity}",
            raw={"severity": severity, "host": host},
        ))
    return out
