...
def get_previous_run_date(self, current_date: str) -> Optional[str]:
        """Return the run_date immediately before *current_date*, or None."""
    ...
    def get_run_findings_summary(self, run_date: str,
                                  priority_filter: Optional[List[str]] = None
                                  ) -> List[Dict[str, Any]]:
        """Return lightweight rows for regression comparison."""
        q = ("SELECT dedup_key, title, product, priority, score "
             "FROM findings WHERE run_date = ? AND status = 'active'")
        args: list = [run_date]
        rows = self.conn.execute(q, args).fetchall()
        cols = ["dedup_key", "title", "product", "priority", "score"]
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        runs = self.conn.execute(
            "SELECT COUNT(DISTINCT run_date) FROM runs"
        ).fetchone()[0]
        top = self.conn.execute(
            "SELECT MAX(score) FROM findings"
        ).fetchone()[0]
        return {"total_active_findings": total, "runs": runs, "top_score": top}
