...
Stages (mapped to the activity's solution approach):
  ...
  4.5 AI ENRICHMENT      FP classification + smart remediation (Claude API)
...
from . import (
    ai_enrich as ai_mod,
    attackpath, dashboard, dedup,
    enrich, filter as filt,
    history, normalize, output,
    rank as rank_mod, remediation,
    score as score_mod,
)
...
def run(reports_dir: str, config: Config, out_dir: str,
    ...
    dedup_result = dedup.deduplicate(
        findings, fuzzy=config.dedup_cfg.get("fuzzy_title", False))
    ...
    print(f"  active={filter_metrics['active']} "
          f"quarantined={filter_metrics['quarantined']}")
    ...
    # ── Stage 4.5: AI enrichment (optional — requires ANTHROPIC_API_KEY) ─
    print("== [4.5/8] AI ENRICHMENT (FP classification + smart remediation) ==")
    ...
    ai_summary_stats = {
        ...
        "p1": sum(1 for f in active_pre_ai
                  if f.score is not None and f.score >= 80),
        "p2": sum(1 for f in active_pre_ai
                  if f.score is not None and 60 <= f.score < 80),
        "p3": sum(1 for f in active_pre_ai
                  if f.score is not None and 40 <= f.score < 60),
    ...
    for product in products:
        ...
        paths = attackpath.build_attack_paths(
            findings, product, config.product(product))
    ...
    for f in findings:
        if f.status == "active" and not f.remediation_suggestions:
    ...
    summary = RunSummary(
        ...
        products=[p for p in products
                  if any(f.product == p for f in findings)],
        raw_findings=dedup_metrics["raw"],
        unique_findings=dedup_metrics["unique"],
    ...
    output.write_ranked_csv(
        os.path.join(out_dir, "ranked_findings.csv"), ranked)
    output.write_ranked_json(
        os.path.join(out_dir, "ranked_findings.json"), ranked)
    output.write_analytics_csv(
        os.path.join(out_dir, "analytics.csv"), ranked)
    output.write_top_actions_md(
        os.path.join(out_dir, "top_actions.md"), ranked, summary)
    output.write_tickets_md(
        os.path.join(out_dir, "tickets_ready.md"), ranked,
    ...
    output.write_metrics_json(
        os.path.join(out_dir, "noise_reduction.json"), noise)

    # history (risk-over-time chart)
    ...
    dashboard.build_dashboard(
        ...
        findings, ranked, summary, all_paths, history_map,
        quarantine_list,
        executive_brief=ai_result.get("executive_brief", ""),
    )

    print("\n[OK] pipeline complete — outputs in", out_dir)
    print("   ranked_findings.csv · analytics.csv · top_actions.md · "
          "tickets_ready.md · noise_reduction.json · "
          "risk_dashboard.html · history.db")
...
def main(argv: Optional[List[str]] = None) -> None:
    parser = argparse.ArgumentParser(
        description="Risk Prioritization & Deduplication pipeline (shared core)")
    parser.add_argument("--reports", required=True,
                        help="directory with <product>_<scanner>.json reports")
    parser.add_argument("--config",  default=None,
                        help="path to config.json")
    parser.add_argument("--out",     default="out",
                        help="output directory")
    parser.add_argument("--products", default=None,
                        help="comma-separated product filter")
    parser.add_argument("--skip-enrich", action="store_true",
                        help="skip live threat-intel lookups")
    parser.add_argument("--skip-ai", action="store_true",
                        help="skip AI enrichment (ignores ANTHROPIC_API_KEY)")
    parser.add_argument("--searchsploit", action="store_true",
                        help="use exploit-db CSV for exploit lookups")
